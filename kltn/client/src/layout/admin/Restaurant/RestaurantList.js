      ...
      <div className="container-xxl">
        <div className="align-items-end row g-4 mb-4" data-aos="fade-down">
          ...
          <div className="col-12 col-xl-auto">
            <button onClick={() => setCurrentView('add-restaurant')} className="btn btn-primary-soft">
              <i className="fa fa-plus me-2"></i> Thêm Nhà Hàng
            </button>
          </div>
        </div>
        <ul className="restaurant-list">
          {currentItems.map((restaurant) => (
            <li key={restaurant._id} className="border-0 card card-hover flex-fill overflow-hidden rounded-3 shadow-sm w-100 card-hover-bg mb-3">
                ...
                <div className="g-0 h-100 row">
                  <div className="col-lg-3 col-md-5 col-sm-4 col-xxl-2 position-relative">
                    <div className="card-image-hover dark-overlay h-100 overflow-hidden position-relative">
                      <img style={{width:'223px', height:'239px'}}  src={restaurant.image} alt={restaurant.name} className="image-tour" />
                    </div>
                  </div>
                    ...
                    <div className="d-flex flex-column h-100">
                      <div className="d-flex end-0 gap-2 me-3 mt-3 position-absolute top-0 z-1">
                        <button
                          ...
                          onClick={() => handleEdit(restaurant.touristSpotId, restaurant._id)}
                        ...
                        <button
                          style={{ border: 'none', background: 'none' }}
                          onClick={() => handleDelete(restaurant.touristSpotId, restaurant._id)}
                          className="align-items-center bg-light btn-icon d-flex justify-content-center rounded-circle text-primary"
                          data-bs-toggle="tooltip"
                          data-bs-placement="top"
                      ...
                      <p className="mt-1 fs-14 text-muted description">{restaurant.description}</p>
                      ...
                      <div className="d-flex flex-column gap-3 gap-lg-2 gap-xl-3 mt-auto z-1">
                        ...
                        <i class="fa-solid fa-dollar-sign"></i>                       
                          <span>Giá: {restaurant.price}</span>
                        </a>
                        <a href={`tel:${restaurant.address}`} className="d-flex gap-2 align-items-center fs-13 fw-semibold">
                        ...
                        {touristSpots[restaurant.touristSpotId] && (
                          <a className="d-flex gap-2 align-items-center fs-13 fw-semibold">
                            ...
                            <span>Địa điểm du lịch: {touristSpots[restaurant.touristSpotId].name}</span>
                          </a>
                        )}
                      </div>
                    </div>
